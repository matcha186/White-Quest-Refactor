...
- [x] デバッグ用ダイス目設定（`player-dice-select`/`enemy-dice-select`）が反映されない問題を修正 — `index.html`側のUIコメントアウトは解除済みだったが、`script.js`側の要素参照とchangeイベントリスナーがコメントアウトされたままだった。両方を有効化し、Playwrightで選択値が`playerDiceNum`/`enemyDiceNum`に反映されることを確認
- [x] バグ19: 友情コンボの召還アイコン（`summon-icon`）が表示されない問題を修正 — `nakamu5Action()`内のアイコンパスのみ先頭に`/`が付いた絶対パスになっており、`file://`で直接開いた場合にドライブルート起点で解決されて画像が見つからなかった。相対パスに統一し、`file://`で友情コンボを実行してアイコンが正しく読み込まれることを確認
