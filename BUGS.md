...
- [A. ロジック / ゲームプレイに影響するバグ](#a-ロジック--ゲームプレイに影響するバグ)
  ...
  19. [召還アイコンが表示されない（ユーザー報告）](#19-友情コンボの召還アイコンsummon-iconが表示されないユーザー報告修正済み)
...
### 19. 友情コンボの召還アイコン（summon-icon）が表示されない（ユーザー報告・修正済み）

**該当箇所**: `nakamu5Action()` 内の `characters` マップ（アイコンパス）
...
**原因**: この4つのアイコンパスだけ先頭に `/` が付いた絶対パスになっていました。他の画像パス（`imageMap`, `CHARACTERS` テーブルなど）はすべて `img/cards/...` のような相対パスで統一されています。`/img/cards/WarriorIcon.png` のような絶対パスは「サイトのルート」を起点に解決されるため、`index.html` を `file://` で直接開いた場合（このプロジェクトのようにビルド不要で直接開くケースでは一般的）は Windows のドライブルート（例: `file:///C:/img/cards/WarriorIcon.png`）を指してしまい、実際のファイルが存在せず画像が読み込めません（壊れた画像アイコンが表示される）。サブディレクトリ配信の場合も同様にルート起点でずれます。

**影響**: Nakamuの友情コンボで誰かを召喚すると、画面上部に表示されるはずの召喚キャラのアイコン（`#player-summon-icon`/`#enemy-summon-icon`）が常に壊れた画像として表示されてしまいます。

**修正方法**: 4つのアイコンパスすべてから先頭の `/` を削除し、他の画像パスと同じ相対パス（`img/cards/...`）に統一しました。`file://` で直接開いた状態で友情コンボを実行し、アイコンが正しく読み込まれることを確認済みです。
